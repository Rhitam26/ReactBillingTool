    ...
    try {
      ...
      if (foundProduct) {
        ...
        if (invoiceType === 'secondary') {
          ...
          switch (customerType) {
            case 'Gold':
              price = foundProduct[19]; // kaccha_sales_price_1
              break;
            case 'Silver':
              price = foundProduct[18]; // kaccha_sales_price_2
              break;
            case 'Platinum':
              price = foundProduct[20]; // kaccha_sales_price_3
              break;
            case 'Diamond':
              price = foundProduct[20]; // kaccha_sales_price_4
              break;
            default:
              price = foundProduct[16]; // kaccha_default_price
          }
        } else if (invoiceType === 'standard') {
          // Standard invoice pricing
          switch (customerType) {
            case 'Gold':
              price = foundProduct[10]; // price_gold
              break;
            case 'Silver':
              price = foundProduct[9]; // price_silver
              break;
            case 'Platinum':
              price = foundProduct[11]; // price_platinum
              break;
            case 'Diamond':
              price = foundProduct[11]; // price_diamond
              break;
            default:
        ...
        setProducts((prevProducts) => [
          ...prevProducts,
          {
            ...
            current_stock: foundProduct[6],
            default_price: foundProduct[7],
            price_gold: foundProduct[10],
            price_silver: foundProduct[9],
            price_diamond: foundProduct[11],

            kaccha_barcode: foundProduct[15],
            kaccha_purchase_price: foundProduct[16],
            kaccha_default_price: foundProduct[17],
            kaccha_sales_price_1: foundProduct[18],
            kaccha_sales_price_2: foundProduct[19],
            kaccha_sales_price_3: foundProduct[20],
