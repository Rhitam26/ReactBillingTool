...
class PrimaryProduct(BaseModel):
    ...
    kacchaBarcode: Optional[str]
    kacchaPurchasePrice: Optional[str]
    kacchaDefaultSalesPrice: Optional[str]  # Changed from kacchaSalesPrice
    kacchaSilverPrice: Optional[str]
    kacchaGoldPrice: Optional[str]
    kacchaDiamondPrice: Optional[str]
    linkedToPrimary: Optional[str]  # Added this field
...
async def update_google_sheets(products_data):
    # try:
    sheet = await authenticate_google_sheets('g-auth.json', 'product')
    range_name = 'product!A1:Z'
    ...
    for product in products_data['products']:
        if product.get('type') == 'primary':
            row = [
                ...
                product.get('kacchaBarcode', ''),
                product.get('kacchaPurchasePrice', ''),
                product.get('kacchaDefaultSalesPrice', ''),
                product.get('kacchaSilverPrice', ''),
                product.get('kacchaGoldPrice', ''),
                product.get('kacchaDiamondPrice', ''),
                product.get('linkedToPrimary', '')
    ...
    SPREADSHEET_ID = '1YvUhfiX6oBVpl67x7gD4iiChZPis17aNgMVSSEHRak4'
...
async def batch_create_products():
    try:
        ...
        if not products_data or 'products' not in products_data:
            return jsonify({
                "status": "error",
                "message": "Invalid request format. Expected 'products' array."
            }), 400
        ...
        if not products_data['products']:
            return jsonify({
                "status": "error",
                "message": "No products provided for creation."
            }), 400
